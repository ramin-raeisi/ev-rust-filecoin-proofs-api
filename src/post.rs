...
use sds::service::storage::download::set_download_start_time;
...
fn generate_winning_post_inner<Tree: 'static + MerkleTreeTrait>(
    ...
    info!("winning init p_aux {} sds-time {:?}", replicas.len(), timestamp);
...
fn generate_window_post_inner<Tree: 'static + MerkleTreeTrait>(
    ...
    info!("window init p_aux {} sds-time {:?}", replicas.len(), timestamp.elapsed());
